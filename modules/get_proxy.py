...
url1 = 'https://api.pro'
r = requests.get(f'{url1}xyscrape.com/v2/?request=getproxies&protocol=socks5&timeout=3000&country=all&simplified=true&ssl=all&anonymity=elite,anonymous')
proxy_list = re.findall(r'\d+\.\d+\.\d+\.\d+:\d+', r.text)
all_proxy = ['socks5://' + i for i in proxy_list]
r = requests.get(f'{url1}xyscrape.com/v2/?request=getproxies&protocol=socks4&timeout=3000&country=all&simplified=true&ssl=all&anonymity=elite,anonymous')
proxy_list = re.findall(r'\d+\.\d+\.\d+\.\d+:\d+', r.text)
